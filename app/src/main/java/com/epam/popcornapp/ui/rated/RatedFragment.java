...
public class RatedFragment extends BaseMvpFragment implements
        MainPresenterView,
        BaseMoreView,
    ...
    @InjectPresenter
    RatedMoviePresenter ratedMoviePresenter;

    @InjectPresenter
    RatedTvPresenter ratedTvPresenter;
    ...
    public static RatedFragment getInstance() {
    ...
    public void onActivityResult(final int requestCode, final int resultCode, final Intent data) {
        ...
        if (resultCode == Constants.ChangeType.RATED) {
            refresh();
        }
    }

    private void initSwimmingLines() {
        movieSwimmingView.createSkeletonTiles(SkeletonTileType.RATING);
        tvSwimmingView.createSkeletonTiles(SkeletonTileType.RATING);
    ...
    @Override
    public void refresh() {
        ratedMoviePresenter.refreshData();
        ratedTvPresenter.refreshData();
    }
    ...
    public void onMoreClicked(@TileType final String type) {
        if (type.equals(TileType.MOVIE)) {
            mainPresenter.onMorePressed(RatedMoviePresenter.class.getName());
        } else {
            mainPresenter.onMorePressed(RatedTvPresenter.class.getName());
        }
    }
    ...
    public void setMovieSwimmingLine(final SwimmingLineItem item) {
        ...
        movieSwimmingView.setData(
                SkeletonTileType.RATING, item.getTitle(), item.getTileList());
    }
